...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.getElementById('quickSaveBtn')?.addEventListener('click', saveConfig);
  ...
  document.getElementById('quickExcelBtn')?.addEventListener('click', exportExcel);
  ...
  document.addEventListener('click', (event) => {
    if (event.target.closest('.chip, .teacher-grade-name, .block-cell, .room-class-chip, .room-grade-name, .remove-room, .remove-teacher, .remove-assignment, .add-assignment, #addRoomBtn, #addTeacherBtn, .room-fixed-first')) {
...
function validateManualBoard() {
  if (!lastResult) {
    alert('먼저 빈 수동판을 만들거나 자동안을 가져오세요.');
    return;
  }
...
function clearSelectedTeacherBoard() {
  ...
  lessons.forEach((lesson) => {
    ...
    (lastResult.unplacedLessons ||= []).push(makeUnplacedItemFromLesson(lesson, '선택 전담 빈판'));
  });
...
function renderRoomAssignments() {
  ...
  const rooms = getRoomNamesFromConfig();
  const saved = Array.isArray(appConfig.roomAssignments) ? appConfig.roomAssignments : [];

  if (!rooms.length) {
    area.innerHTML = '<div class="empty-guide">특별실을 먼저 등록하세요.</div>';
    return;
  }

  area.innerHTML = rooms.map((roomName) => {
...
function makeAssignmentHtml(teacher, assignmentIndex) {
  ...
  return `
    <div class="assignment-card" data-assignment-index="${assignmentIndex}">
...
async function saveConfig() {
  ...
  setAutoSaveStatus(result.ok ? '저장됨' : '저장 실패');
  alert(result.ok ? '설정이 저장되었습니다.' : '설정 저장에 실패했습니다.');
}
...
function swapLessonCells(sourceLesson, targetLesson) {
  ...
  const sourceCheck = canPlaceDragLesson(sourceLesson, sourceTarget, null);
  const targetCheck = canPlaceDragLesson(targetLesson, targetTarget, null);
  if (!sourceCheck.ok || !targetCheck.ok) {
    lastResult = snapshot;
    alert(sourceCheck.message || targetCheck.message || '교환할 수 없습니다.');
    renderAllResultViews();
    return true;
  }
...
function showPlacementHints(lesson) {
  ...
  area.querySelectorAll('td[data-teacher]').forEach((cell) => {
    const target = {
      teacher: cell.dataset.teacher,
      dayIndex: Number(cell.dataset.dayIndex),
      period: Number(cell.dataset.period)
    ...
    const canSwap = targetLesson?.value && canSwapLessonHint(lesson.sourceLesson || lesson, targetLesson);
    cell.classList.add(result.ok || canSwap ? (result.warning ? 'placement-warn' : 'placement-ok') : 'placement-bad');
    if (canSwap && !result.ok) cell.classList.add('placement-swap');
    cell.title = result.message || '';
  });
...
function clearDropTarget() {
  document.querySelectorAll('.placement-target').forEach((cell) => cell.classList.remove('placement-target'));
}
...
function showDropTarget(cell) {
  ...
  const duration = Number(activeDragLesson.lesson.duration || 1);
  ...
  for (let i = 0; i < duration; i += 1) {
    ...
    targetCell?.classList.add('placement-target');
  }
}
...
function getLessonUnitFromCell(teacherCode, dayIndex, period) {
  ...
  return {
    ...
    subject: cell.subject || extractSubjectFromValue(cell.value),
...
function clearLessonUnit(lesson) {
  const teacher = findResultTeacher(lesson.teacher);
  for (let i = 0; i < lesson.duration; i += 1) {
    teacher.grid[lesson.period - 1 + i][lesson.dayIndex] = {
      dayIndex: lesson.dayIndex,
      period: lesson.period + i,
      value: '',
      isBlocked: false,
      roomName: '',
      classCode: '',
  ...
  if (lesson.roomName) {
    ...
    if (room) {
      for (let i = 0; i < lesson.duration; i += 1) {
        const cell = room.grid[lesson.period - 1 + i][lesson.dayIndex];
        if (!cell) continue;
  ...
  area.innerHTML = rooms.map((room) => `
    <div class="room-table">
      ...
      <table>
        ...
        <tbody>
          ${room.grid.map((row, periodIndex) => `
            <tr>
              <th>${periodIndex + 1}</th>
              ${row.map((cell, dayIndex) => {
                const movable = canDragRoomCell(cell);
                return `<td class="${cell.isBlocked ? 'blocked' : ''} room-value-cell ${movable ? 'room-movable-cell' : ''}" data-room-name="${escapeAttr(room.roomName)}" data-day-index="${dayIndex}" data-period="${periodIndex + 1}" ${movable ? 'draggable="true"' : ''}>${escapeHtml(cell.isBlocked ? '' : cell.value || '')}</td>`;
              }).join('')}
            </tr>
          `).join('')}
        </tbody>
...
function canDragRoomCell(cell) {
  return !!getMovableRoomEntry(cell);
}
...
function moveRoomCell(source, target) {
  ...
  const sourceCell = sourceRoom.grid?.[Number(source.period) - 1]?.[Number(source.dayIndex)];
  const targetCell = targetRoom.grid?.[Number(target.period) - 1]?.[Number(target.dayIndex)];
  if (!sourceCell || !targetCell || sourceCell.isBlocked || targetCell.isBlocked) return;

  const movingEntry = getMovableRoomEntry(sourceCell);
  if (!movingEntry) {
    alert('전담과 연계된 특별실 수업은 여기서 이동할 수 없습니다.');
    return;
  }

  const targetEntries = Array.isArray(targetCell.entries) ? targetCell.entries : [];
  if (targetEntries.length || String(targetCell.value || '').trim()) {
    alert('대상 칸이 비어 있을 때만 이동할 수 있습니다.');
    return;
  }

  const classCode = String(movingEntry.classCode || '').trim();
  if (hasClassAtManualTime(classCode, Number(target.dayIndex), Number(target.period))) {
    alert('해당 시간에 같은 반 전담 수업이 있어 이동할 수 없습니다.');
    return;
  }

  if (hasClassInRoomAtManualTimeExcept(classCode, Number(target.dayIndex), Number(target.period), {
    roomName: String(source.roomName),
    dayIndex: Number(source.dayIndex),
  ...
  sourceCell.entries = [];
  sourceCell.value = '';
  targetCell.entries = [{ ...movingEntry }];
  targetCell.value = String(movingEntry.value || movingEntry.classCode || '').trim();
...
function resetTimetableViews() {
  ...
  timetableArea.textContent = '빈 수동판을 만들거나 시간표 작성을 누르면 전담시간표가 표시됩니다.';
  ...
  unplacedArea.textContent = '빈 수동판을 만들거나 시간표 작성 후 확인할 수 있습니다.';
  updateSummary();
}
...
function renderUnplaced(items, areaId = 'unplacedArea') {
  const area = document.getElementById(areaId);
  if (!items.length) {
    area.textContent = '미배치 수업 없음';
    return;
  }

  area.innerHTML = items.map((item, index) => `
    <div class="unplaced-item" data-unplaced-index="${index}" draggable="true">
      <div class="unplaced-main">
        <strong>${escapeHtml(item.teacher)}</strong>
        <span>${escapeHtml(item.subject)} ${escapeHtml(item.classCode)}</span>
        <span>${escapeHtml(item.unitType)}</span>
      </div>
      <div class="unplaced-reason">${escapeHtml(item.roomName || '교실')} · ${escapeHtml(item.reason)}</div>
      <div class="manual-place-row">
        <select class="manual-day">
          ${DAYS.map((day, dayIndex) => `<option value="${dayIndex}">${day}</option>`).join('')}
        </select>
        <select class="manual-period">
          ${makeManualPeriodOptions(item)}
        </select>
        <button type="button" class="manual-place-btn">배치</button>
      </div>
    </div>
  `).join('');

  area.querySelectorAll('.manual-place-btn').forEach((button) => {
    button.addEventListener('click', () => {
      const card = button.closest('.unplaced-item');
      const index = Number(card.dataset.unplacedIndex);
      const dayIndex = Number(card.querySelector('.manual-day').value);
      const period = Number(card.querySelector('.manual-period').value);
      manuallyPlaceUnplaced(index, dayIndex, period);
    });
  });

  area.querySelectorAll('.unplaced-item[draggable="true"]').forEach((card) => {
    card.addEventListener('dragstart', (event) => {
      const index = Number(card.dataset.unplacedIndex);
      const item = lastResult?.unplacedLessons?.[index];
      if (!item) {
...
function manuallyPlaceUnplaced(index, dayIndex, period, targetTeacher = '', askConfirm = true) {
  ...
  if (item.teacher === '특별실') {
    ...
    if (!roomCheck.ok) {
      ...
      lastResult.unplacedLessons.splice(index, 0, item);
      return;
    }
...
function hasClassInRoomAtManualTimeExcept(classCode, dayIndex, period, exceptCell) {
  ...
  return (lastResult.rooms || []).some((room) => {
    ...
    if (exceptCell &&
      ...
      Number(period) === Number(exceptCell.period)) {
      return false;
    }
    const cell = room.grid[period - 1][dayIndex];
    if (!cell) return false;
